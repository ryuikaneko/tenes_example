...
#    parser.add_argument('-hi',metavar='hi',dest='hi',type=np.float,default=1e10,help='set hi (default 1e10 ~ infinity)')
    parser.add_argument('-hi',metavar='hi',dest='hi',type=np.float,default=0.1,help='set hi (default 0.1)')
    parser.add_argument('-hf',metavar='hf',dest='hf',type=np.float,default=0.1,help='set hf (default 0.1)')
    parser.add_argument('-tau',metavar='tau',dest='tau',type=np.float,default=5.0,help='set tau (max time, default 5.0)')
...
#    print(ret)
    ...
    norm = [(np.dot(np.conjugate(ret[i]),ret[i])).real for i in range(Nsteps)]
    print("ene0",ene0)
    print("ene",ene)
    print("mz0mz1",mz0mz1)
    print("mx0mx1",mx0mx1)
...
#
## do not normalize by vmax (--> div 4 for S.S-h.S)
    time_steps_4 = [timei+i*(timef-timei)/(Nsteps-1) for i in range(Nsteps)]
    print("time_steps_4",time_steps_4)
#
## normalize by (pi/4) (--> div pi for S.S-h.S)
    time_steps_pi = [(timei+i*(timef-timei)/(Nsteps-1))/np.pi*4.0 for i in range(Nsteps)]
    print("time_steps_pi",time_steps_pi)
#
## normalize by 1/4 (--> div 1 for S.S-h.S)
    time_steps = [(timei+i*(timef-timei)/(Nsteps-1))*4.0 for i in range(Nsteps)]
...
#    plt.xlabel("$t$")
    plt.plot(time_steps_pi,mx0mx1)
    plt.xlabel("$t/\pi$")
#    plt.plot(time_steps_4,mx0mx1)
#    plt.xlabel("$t/4$")
...
#    fig103.suptitle("mz0mz1/$h_f$")
    fig103.suptitle("mz0mz1")
#    plt.plot(time_steps,np.array(mz0mz1,dtype=np.float)/field_f)
#    plt.plot(time_steps,np.array(mz0mz1,dtype=np.float))
#    plt.xlabel("$t$")
#    plt.plot(time_steps_pi,np.array(mz0mz1,dtype=np.float)/field_f)
    plt.plot(time_steps_pi,np.array(mz0mz1,dtype=np.float))
    plt.xlabel("$t/\pi$")
#    plt.plot(time_steps_4,np.array(mz0mz1,dtype=np.float))
#    plt.xlabel("$t/4$")
...
#    plt.xlabel("$t$")
    plt.plot(time_steps_pi,-np.log(loschmidt_echo)/N)
    plt.xlabel("$t/\pi$")
#    plt.plot(time_steps_4,-np.log(loschmidt_echo)/N)
#    plt.xlabel("$t/4$")
